...
public class DBHandler extends SQLiteOpenHelper{
    ...
    //Name der Datenbank
    private static String DATABASE_NAME = "WOMS_Hausaufträge";
    ...
    private String KEY_AuftragsID ="auftragsNr";
    ...
    public void onCreate (SQLiteDatabase db){
        String CREATE_AUFNAHME_TABLE = "CREATE TABLE" + TABLE_AUFNAHME + "("
                +KEY_OBJEKT+"char(36) PRIMARY KEY,"
                +KEY_RAUM+"char(5) PRIMARY KEY,"
                +KEY_DATUM+"date(10),"
                +KEY_Aufnehmer+"char(40),"
                +KEY_MANGELID+"number(5) FOREIGN KEY,"
                +")";
        db.execSQL(CREATE_AUFNAHME_TABLE);
    ...
    public JSONArray auftraegeAsJSON() throws JSONException {
        ...
        if (cursor.moveToFirst()){
            do {
                ...
                auftrag.put("status_flag",cursor.getString(1).toString());
                auftrag.put("prio",cursor.getString(2).toString());
                auftrag.put("raum",cursor.getString(3).toString());
                auftrag.put("kategorie",cursor.getString(4).toString());
